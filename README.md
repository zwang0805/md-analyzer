...
| `md_analyzer.py` | `mda` | `MDAnalyzer` class — all trajectory analysis logic |
| `web_app.py` | `mda` | Flask web UI for `MDAnalyzer`; launch from the command line |
...
## Part 2 — Trajectory Analysis (command-line)

`md_analyzer.py` provides the `MDAnalyzer` class for analysing simulation output from Part 1.
It uses **MDAnalysis**, **ProLIF**, and **scikit-learn** and requires the separate `mda` conda environment.

For a point-and-click browser interface, see **Part 3 — Web UI**.
...
pip install flask          # required by web_app.py (Part 3)
```
...
```bash
python -c "
import MDAnalysis, prolif, pandas, sklearn, flask
...
The topology and trajectory **must have the same number of atoms**. When generated by `run_md.py`, use `com.prmtop` together with the matching `.dcd` file.
...
| `remove_subunits` | Selection string to discard atoms (e.g. `'chainID B'`); pass `'no'` to skip |
| `solv_selection` | Comma-separated solvent resnames (e.g. `'HOH,WAT'`); pass `'no'` to use defaults |
| `lig_selection` | Comma-separated ligand resnames (e.g. `'UNK'`); pass `'no'` to use auto-detection |
| `dt` | Override trajectory timestep (ps); leave empty to use value from file header |
...
#### 7 — Bond Angle over Trajectory

Calculates the angle formed by three atoms and plots it over time.

```python
ana.calculate_angle_trajectory(atom_label='3044,3045,3046')
```

| Parameter | Description |
|-----------|-------------|
| `atom_label` | Three comma-separated 0-based atom indices |

Output: `<work_dir>/analysis/<pdb>_angle.png` and `_angle.csv`

---

#### 8 — Pairwise RMSD Heatmap
...
> **Note:** Computation time scales as O(N²) with the number of frames. For large trajectories, use a coarser step when loading the trajectory.

---

#### 9 — Secondary Structure (DSSP)
...
```python
ana.DSSP(residues='all', dssp_step=50)
```
...
| Bond angle | `*_angle.png`, `*_angle.csv` |
...
**Topology has no segid information**

If the topology (e.g. `.prmtop`) does not define chain segment IDs, `MDAnalyzer` falls back to numbering chains 0, 1, 2, … automatically. No action is required.

**`fit_rot_trans` raises `NoDataError: This Universe has no coordinates`**

This occurs when a topology-only format (`.prmtop`, `.psf`) is used. The reference structure for alignment is now built from the first trajectory frame automatically — this is handled internally and requires no user action.
...
`web_app.py` wraps `MDAnalyzer` in a Flask web application so all analyses can be run from a browser with no Python required after startup.

### Features

- Dark-themed single-page interface with sidebar navigation
- Load topology and trajectory files from any `working_directory/` subdirectory
- All 10 analysis methods accessible via point-and-click with editable parameters
- Real-time log panel (Server-Sent Events) shows progress and errors as they happen
- Output file browser with in-page image preview and file download

### Starting the server

```bash
# Default: port 5050, all interfaces
/path/to/envs/mda/bin/python web_app.py

# Custom port / host
/path/to/envs/mda/bin/python web_app.py --port 8080 --host 127.0.0.1

# Debug mode (auto-reloads on code changes)
/path/to/envs/mda/bin/python web_app.py --debug
```

Then open `http://localhost:5050` (or the port you chose) in a browser.

> **Tip:** If `conda run -n mda python` resolves to a different environment on your system, use the full path `/path/to/miniconda3/envs/mda/bin/python` instead.
...
|-------|----------------|
| Load Trajectory | Working directory, topology file, trajectory file, timestep (ps), atoms to remove, solvent / ligand resnames, apo flag |
| System Info | Displays atom count, frame count, segment IDs, ligand selection string |
| RMSD | Step (frames) |
| RMSF | Mode (ligand / protein), selection string, alignment string, step |
| Distance | Two atom selection strings (comma-separated), step |
| Dihedral Angle | Type (ligand atom IDs / protein selection), atom label, step |
| Bond Angle | Three 0-based atom indices |
| DSSP | Residue range, step |
| RMSD Heatmap | (no parameters — press Run) |
| Pro-Lig Fingerprint | Ligand selection string, type (small molecule / peptide) |
| Clustering | Start / end time, number of clusters, atom selection mode |
| Extract Frames | Start / end time, number of frames |
| Output Files | Lists all files in `analysis/` with preview for images and download links |

### Architecture

`web_app.py` starts a single Flask process (threaded mode). Each analysis runs in a background daemon thread so the HTTP server stays responsive. All `print()` output from `MDAnalyzer` and all interactive `input()` prompts are intercepted and forwarded to the browser log panel via a persistent SSE connection at `/api/stream`.

The analysis methods themselves are unchanged — `web_app.py` only wraps them.
